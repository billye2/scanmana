...
/**
 * Take a signal into the manual book: the same buy-stop the deck shows (box top,
 * else the bare pivot) with the box bottom as the stop. A watchlist row supplies
...
export async function POST(req: Request) {
  ...
  const r = await takeSignal(userId, ticker, trigger, stop);
