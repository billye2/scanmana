...
**Two more measured changes (1.3.2, 2026-09-20):** `MAX_DIST_FROM_HIGH` 0.15 → 0.05 (the one screen knob that moved the 10-day breakout rate: 70% within 5% vs 53% further out; deck shrinks from ~55 to ~47) and paper position size scaled by the tape (`lib/tape.ts`: full when index Bullish and breadth ≥ 50%, half with one signal, quarter with none; the same setups averaged +0.11R Bullish vs −0.11R not). Watch on /research over the next month whether the 5% cut holds out of sample, and on /paper whether the tape sizing lifts dollar expectancy (R per trade is unchanged by sizing, dollars are not). **Livermore no longer vetoes the verdict (1.3.0, 2026-09-20)** — first measured result acted on; expect roughly a third more Wait cards and therefore more auto paper orders from the 2026-09-21 scan on. **Research layer shipped 2026-09-20 (1.2.9).** First unattended run is the night of 2026-09-21 (Monday's 04:00 UTC scan chains it; the 05:30 catch-up gives it the full budget). Check the morning after: `research_runs` has six rows with `ok = true` for the new `scan_date`, `/research` shows the new date in the job log, the deck cards carry the like-this / theme lines, the market strip has the breadth line. Then look at: `research_splits` rows in `review` (approve real splits, ignore news days: the top of the list is the ambiguous ones); a cluster alias or two on `/research` (themes). Notes: the Python function is at `/api/research_job` (public path in `lib/auth.ts`, checks CRON_SECRET itself; preview has no CRON_SECRET and relies on deployment protection, `vercel curl` bypasses it); local runs are `npm run research -- --jobs outcomes,sweep --force` (needs `.venv`, see `requirements.txt`); the sweep grid's loosest values must stay equal to `LOOSE_LIMITS` in `lib/screen.ts` (tests/py/test_sweep.py asserts it); confirmed splits (`research_splits.status='auto'`) rescale bars in `loadBars` for the scan and the charts. First reading of the data (149 sessions, one weak tape): Minervini agreement lifts the 10-day breakout rate most (67% vs 59%), Darvas agreement next (65% vs 56%), Livermore agreement does not (62% vs 66%, because his "chasing" objection flags names already running); every framework's box-top-buy / 10-low-trail R is slightly negative in this sample.
