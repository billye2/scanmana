...
_Last updated: 2026-09-23 (1.3.4). Read this first when picking up the project._
...
nightly (`/research`); rule changes are made from its numbers, registered in
`lib/research-story.ts` so the page scores them, and logged in the config
comment, the commit and here. Full product decisions and thresholds: see
...
1. `/research`: no red line under the Scorecard heading (a failed job or no run for the 2026-09-21 scan shows there); Maintenance summary says "6 of 6 jobs ok". If a job is missing, `npm run research -- --jobs <job> --force` reruns it locally with the same code. The "since 09-21" tiles and the Rule changes table stay at n=0 until about 2026-10-05 (ten sessions to label) and grey until 30 rows — that is expected, not a bug.
...
**Sweep headline as of 2026-09-20 (offline render):** MIN_ADR_PCT — 3.5% now at 70%, 5% would have been 73% (n=4,442). In-sample, and the 5% limit only took effect 2026-09-21; let the Rule changes table score that first before touching ADR.

**Deferred:** a per-theme hit rate on `/research` (needs a few weeks of `research_cluster_history`); a Live overlay on `/paper` (Finnhub budget is at 59/60 per minute already); an equity-curve chart (data in `GET /api/paper/book` → `stats.equityCurve`); stats split by which framework said Wait (the raw material is `scan_history.objections` × `research_outcomes`).
...
- A rule change made from `/research` cites the number in the `lib/config.ts` comment, the commit body and this file, so the next reader can tell a measured setting from an inherited one — and adds an entry to `RULE_CHANGES` in `lib/research-story.ts` (first effective scan date, version, label, lens) in the same commit, so the page scores it.
