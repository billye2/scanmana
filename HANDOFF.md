...
| Access | ✅ single-password gate (`proxy.ts` + `/login`, cookie = HMAC keyed by `APP_PASSWORD`, 1-year). Public: `/api/cron/*` (own secret), manifest, sw.js, icons. Unset `APP_PASSWORD` = open. Change the password to log every device out. |
